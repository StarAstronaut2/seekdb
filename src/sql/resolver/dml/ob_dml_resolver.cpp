                                                   ...
                                                   TableItem *&tbl_item)
{
  ...
  const int64_t NUM_COLS = 4;
  const char *col_names[NUM_COLS] = {"chunk_id", "chunk_offset", "chunk_length", "chunk_text"};
  ObObjMeta col_types[NUM_COLS];
  ObAccuracy col_accuracies[NUM_COLS];
  ...
  CK (OB_LIKELY(T_AI_SPLIT_DOCUMENT_EXPRESSION == parse_tree.type_));
  CK (OB_LIKELY(3 == parse_tree.num_child_));
  
  if (OB_SUCC(ret)) {
    // Extract alias from children_[2], may be NULL
    ParseNode *alias_node = parse_tree.children_[2];
    if (OB_NOT_NULL(alias_node)) {
      alias_name.assign_ptr((char *)(alias_node->str_value_), static_cast<int32_t>(alias_node->str_len_));
    }

    // Resolve content expression (children_[0])
    if (OB_ISNULL(parse_tree.children_[0])) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("content expr is null", K(ret));
    } else {
      OX (params_.is_resolve_table_function_expr_ = true);
      OZ (resolve_sql_expr(*(parse_tree.children_[0]), content_expr));
      OX (params_.is_resolve_table_function_expr_ = false);
    }
  }

  if (OB_SUCC(ret)) {
    // Resolve params expression (children_[1]), may be NULL
    if (OB_NOT_NULL(parse_tree.children_[1])) {
      OX (params_.is_resolve_table_function_expr_ = true);
      OZ (resolve_sql_expr(*(parse_tree.children_[1]), params_expr));
      OX (params_.is_resolve_table_function_expr_ = false);
    }
  }

  CK (OB_NOT_NULL(content_expr));
  OZ (content_expr->deduce_type(session_info_));
  if (OB_NOT_NULL(params_expr)) {
    OZ (params_expr->deduce_type(session_info_));
  }

  if (OB_SUCC(ret)) {
    // Create the compound expression: call ai_split_document(content, params)
    // We'll build a raw expression that wraps both as a sys function
    ObOpRawExpr *func_expr = NULL;
    OZ (params_.expr_factory_->create_raw_expr(T_FUN_SYS_AI_SPLIT_DOCUMENT, func_expr));
    CK (OB_NOT_NULL(func_expr));
    OX (func_expr->set_expr_name(ObString(N_AI_SPLIT_DOCUMENT)));
    OZ (func_expr->add_param_expr(content_expr));
    if (OB_NOT_NULL(params_expr)) {
      OZ (func_expr->add_param_expr(params_expr));
    }
    
    // Set result type (LongText - the JSON array output)
    ...
    OX (func_expr->set_expr_type(result_type));
    ...
    if (OB_SUCC(ret)) {
      if (OB_ISNULL(item = stmt->create_table_item(*allocator_))) {
        ...
        if (alias_name.empty() && OB_FAIL(stmt->generate_func_table_name(*allocator_, alias_name))) {
        ...
        OX (item->function_table_expr_ = func_expr);
      }
    }
  }

  // Add 4 column items with fixed names and types
  if (OB_SUCC(ret)) {
    // chunk_id: INT
    col_types[0].set_int();
    col_accuracies[0].set_precision(DEFAULT_PRECISION_FOR_INTEGER);
    col_accuracies[0].set_scale(DEFAULT_SCALE_FOR_INTEGER);
    // chunk_offset: INT
    col_types[1].set_int();
    col_accuracies[1].set_precision(DEFAULT_PRECISION_FOR_INTEGER);
    col_accuracies[1].set_scale(DEFAULT_SCALE_FOR_INTEGER);
    // chunk_length: INT
    col_types[2].set_int();
    col_accuracies[2].set_precision(DEFAULT_PRECISION_FOR_INTEGER);
    col_accuracies[2].set_scale(DEFAULT_SCALE_FOR_INTEGER);
    // chunk_text: VARCHAR
    col_types[3].set_varchar();
    col_types[3].set_collation_type(CS_TYPE_UTF8MB4_BIN);
    col_accuracies[3].set_length(ObAccuracy::DDL_DEFAULT_ACCURACY[ObVarcharType].get_length());

    for (int64_t i = 0; OB_SUCC(ret) && i < NUM_COLS; ++i) {
      ColumnItem *col_item = NULL;
      OZ (resolve_function_table_column_item(*item,
                                              col_types[i],
                                              col_accuracies[i],
                                              ObString(col_names[i]),
